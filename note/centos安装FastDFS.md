...
我们要启动tracker，就修改刚刚看到的`tracker.conf`，并且启动`fdfs_trackerd`脚本即可。
...
```shell
base_path=/tmp
tracker_server=192.168.149.101:22122
```
运行:
```shell
/usr/bin/fdfs_upload_file client.conf /tmp/图片全名
```

查看是否返回文件ID

```txt
group1/M00/00/00/wKiVZVxifXqAP2DlAAMjMd_mNA8715.png
```
...
- 配置
  ...
  --add-module指代的时候刚才fastdfs模块的src路径

- 编译安装

  如果Nginx已经安装过.则只需要编译(注意! 只编译,不要安装!)
